...
// drifts.
//
// Pacing, which is the other thing these numbers decide:
    ...
    turns: [
      {
        prompt: 'tests',
        lines: [
          ['run', '● Read  src/billing/charge.test.ts (96 lines)', 1.3, 750],
          ['run', '● Bash  npm test -- billing', 1.9, 900],
      ...
      },
      {
        prompt: 'findRetry',
        lines: [
          ['run', '● Grep  "retry" (7 files)', 1.6, 700],
          ['run', '● Read  src/webhook/retry.ts (140 lines)', 1.8, 850],
      ...
      },
      {
        prompt: 'extract',
        lines: [
          ['run', '● Read  src/webhook/retry.ts (140 lines)', 1.4, 750],
          ['run', '● Edit  src/webhook/retry.ts', 1.9, 900],
          ['run', '● Bash  npm test -- webhook', 3.0, 900],
          ['ok', '  ✓ 11 passed  (1.8s)', 0.4, 800],
        ],
      },
      {
        prompt: 'tidy',
        lines: [
          ['run', '● Edit  src/webhook/retry.ts', 1.8, 800],
          ['run', '● Edit  src/billing/charge.ts', 1.7, 800],
          ['run', '● Bash  npm run lint', 2.4, 900],
    ...
    turns: [
      {
        prompt: 'refactor',
        lines: [
          ['run', '● Read  src/billing/charge.ts (412 lines)', 1.2, 800],
          ['run', '● Read  src/billing/invoice.ts (268 lines)', 1.0, 700],
          ['run', '● Edit  src/billing/charge.ts', 1.5, 900],
          ['run', '● Edit  src/billing/invoice.ts', 1.2, 850],
          ['run', '● Bash  npm test -- billing', 1.3, 900],
          ['ok', '  ✓ 24 passed  (3.1s)', 0.3, 850],
        ],
      },
      {
        prompt: 'types',
        lines: [
          ['run', '● Bash  npx tsc --noEmit', 1.1, 800],
          ['run', '● Edit  src/billing/invoice.ts', 1.5, 900],
          ['run', '● Edit  src/api/checkout.ts', 1.4, 850],
          ['run', '● Edit  src/jobs/settle.ts', 1.3, 850],
          ['run', '● Bash  npx tsc --noEmit', 0.9, 800],
          ['ok', '  ✓ 0 errors', 0.3, 800],
        ],
      },
      {
        prompt: 'edgeTests',
        lines: [
          ['run', '● Read  src/billing/charge.ts (438 lines)', 1.0, 750],
          ['run', '● Write src/billing/charge.test.ts', 2.1, 1000],
          ['run', '● Bash  npm test -- billing', 1.4, 900],
          ['run', '● Edit  src/billing/charge.test.ts', 1.7, 900],
          ['ok', '  ✓ 31 passed  (4.0s)', 0.3, 850],
        ],
      },
      {
        prompt: 'review',
        lines: [
          ...
          ['run', '● Read  src/billing/charge.ts (438 lines)', 1.3, 800],
          ['run', '● Read  src/billing/invoice.ts (281 lines)', 1.1, 800],
          ['run', '● Edit  src/billing/charge.ts', 1.8, 900],
          ['run', '● Bash  npm test -- billing', 1.3, 900],
          ['ok', '  ✓ 31 passed  (4.1s)', 0.3, 850],
        ],
      },
    ],
  },
    ...
    turns: [
      {
        prompt: 'rename',
        lines: [
          ['run', '● Grep  "Charge" (48 files)', 1.2, 800],
          ['run', '● Edit  src/billing/charge.ts', 1.3, 850],
          ['run', '● Edit  src/api/checkout.ts', 1.2, 850],
          ['run', '● Edit  src/webhook/retry.ts', 1.1, 800],
          ['run', '● Edit  src/jobs/settle.ts', 1.2, 850],
          ['run', '● Bash  npx tsc --noEmit', 1.0, 800],
        ],
      },
      {
        prompt: 'buildAll',
        lines: [
          ['run', '● Bash  npm run build', 1.3, 850],
          ['run', '● Edit  src/billing/invoice.ts', 1.4, 900],
          ['run', '● Edit  src/jobs/settle.ts', 1.3, 850],
          ['run', '● Edit  src/api/checkout.ts', 1.2, 850],
          ['run', '● Bash  npm run build', 1.2, 900],
          ['ok', '  ✓ build passed  (28.4s)', 0.6, 850],
        ],
      },
      {
        prompt: 'fullSuite',
        lines: [
          ['run', '● Bash  npm test', 1.4, 900],
          ['run', '● Edit  src/billing/charge.test.ts', 1.3, 850],
          ['run', '● Edit  src/webhook/retry.test.ts', 1.2, 850],
          ['run', '● Bash  npm test', 1.4, 900],
          ['run', '● Bash  npm run lint', 1.0, 800],
          ['ok', '  ✓ 186 passed  (22.7s)', 0.7, 900],
        ],
      },
    ],
  },
